    ...
    */
    ...
    public function getTransactions(string $address, int $limit = 100,  $min_block_timestamp = 0,  $max_block_timestamp = 3648806636000): array
    {
        return $this->_tron->getManager()
            ->request("v1/accounts/{$address}/transactions/trc20?limit={$limit}&min_block_timestamp={$min_block_timestamp}&max_block_timestamp={$max_block_timestamp}&contract_address={$this->contractAddress}", [], 'get');
    }

    public function getTransactionevent(string $transaction_id): array
    {
        return $this->_tron->getManager()
            ->request("v1/transactions/$transaction_id/events", [], 'get');
    }
